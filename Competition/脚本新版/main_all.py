...
sheet_head = ['项目编号', '项目名称', '项目类型', '所属学校', '项目期限', '所属一级学科',
              '所属二级学科', '立项时间', '结题时间', '指导老师', '职称', '指导老师类型', '负责人', '其他成员', '专业班级',
              '所在学院', '项目分工', '成员类型']
...
def down(LXYear, SchoolName, urlList, progress=gr.Progress()):
    ...
    for url in progress.tqdm(urlList):
        # time.sleep(0.1)
        dataArr.append(getInfo(url))
    # 将二维数组dataArr转换为dataFrame
    dataArr = pd.DataFrame(dataArr)
    dataArr.columns = sheet_head
    dataArr.insert(3, '项目类别', levelList)
    print(dataArr)
    wbResult = openpyxl.Workbook()
    wsR = wbResult.active
    # 将dataArr的column写入excel
    wsR.append(dataArr.columns.to_list())
    print('正在导出为excel')
    # 遍历dataFrame dataArr，并将其写入excel
    for index, row in dataArr.iterrows():
        # time.sleep(0.1)
        wsR.append(row.values.tolist())
    # for row in progress.tqdm(dataArr.values.):
    #     # time.sleep(0.1)
    #     wsR.append(row)
    # 保存到/excel目录下
        ...
        if res.status_code == 200:
            ...
            for tr in table.find_all('tr'):
                for index, td in enumerate(tr.find_all('td')):
                    if index == 3:
                        levelList.append(td.text.strip())
            print(levelList)
...
print('启动成功,请不要关闭此窗口')
demo.queue().launch(server_port=30003, inbrowser=True)
